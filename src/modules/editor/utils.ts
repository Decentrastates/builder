import { BodyShapeRespresentation, Wearable } from 'decentrastates-ecs'
...
export function patchWearables(wearables: Wearable[]) {
  return wearables.map(wearable => {
    ...
    if (wearable.category === 'skin') {
      const alreadyHidden: string[] = [...((wearable as any).hides || [])] // The typing from decentrastates-ecs is wrong and it misses the hides list
